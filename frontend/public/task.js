...
async function addTaskBack(taskData) {
  ...
  async function editTaskBack(task, oldtask) {
    console.log(task);
    console.log(oldtask);
      try {
      const response = await fetch('http://localhost:3000/api/tasks/edit', {
        ...
        body: JSON.stringify({ task: task, oldtask: oldtask })
  ...
  function loadSampleTasks(sampleTasks) {
    ...
    sampleTasks.forEach((task, index) => {
      ...
      createActionsDropdown(actionsCell, task.id);
    });
...
`;
  ...
  function handleEditTaskClick(taskId) {
    ...
    const taskToEdit = sampleTasks.find((task) => task.id === taskId);
  ...
  async function handleEditTaskSubmit(e) {
    ...
    updatedTask.assignedTo = e.currentTarget.querySelector("#assignedToEdit").value.split(",").map((member) => member.trim());
    ...
    await editTaskBack(updatedTask, taskToEdit);
